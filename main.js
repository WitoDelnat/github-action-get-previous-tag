...
exec(`git for-each-ref --sort=-creatordate --count 1 --exclude="refs/tags/*-hotfix.*" --exclude="refs/tags/*-debug.*" --format="%(refname:short)" "refs/tags/${tagPrefix}"`, {cwd: workingDirectory}, (err, tag, stderr) => {
    tag = tag.trim();
